...
def main(state_space_size, action_space_size=3, gamma=0.99, lr=1e-3, min_episodes=20, eps=1, eps_decay=0.995,
    ...
    for episode in range(num_episodes):
        ...
        if episode % checkpoint_inter ==0 and episode!=0:
            ...
            str_hora_agr = str(datetime.now()).replace(' ','_').replace(':','').replace('-','')[0:15]
            path = '/media/nero-ia/ADATA UFD/sim_data/wsh_EVAL'+str(laser_scan_state_type_atual)+str(n_sectors)+'_'+str_hora_inicio_treino

            if not os.path.exists(path):
                os.makedirs(path)

            filename=path+'/wsh_EVAL'+str(laser_scan_state_type_atual)+str(n_sectors)+'_'+str_hora_inicio_treino+'checkpointn_'+str(episode)+'.out'
...
if __name__ == '__main__':
    ...
    for theta_atual,laser_scan_state_type_atual in [
        ...
        filename=path+'/wsh_EVAL'+str(laser_scan_state_type_atual)+str(n_sectors)+'completo_histQ1.out'
        my_shelf = shelve.open(filename,flag = 'n') # 'n' for new
        ...
        filename=path+'/wsh_EVAL'+str(laser_scan_state_type_atual)+str(n_sectors)+'completo.out'
        my_shelf = shelve.open(filename,flag = 'n') # 'n' for new
        for key in dir():
            try:
